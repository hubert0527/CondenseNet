...
parser.add_argument('--kd_w', default=0.5,  type=float)
...
run_name_tokens = args.savedir.split("/")
for w in reversed(run_name_tokens):
    if w!='':
        run_name = w
        break
...
kd_train = pkl.load(open('./kd_data/train.pkl', "rb"))
kd_test = pkl.load(open('./kd_data/test.pkl', "rb"))

class CIFAR10_KD(datasets.CIFAR10):
    def __getitem__(self, idx):
        t = super(CIFAR10_KD, self).__getitem__(idx)
        if self.train:
            kd_rec = kd_train[idx]
        else:
            kd_rec = kd_test[idx]
        ...
        # # Sanity test
        #inputs, target = t
        #if target != kd_rec['targets']:
        #    raise Exception("ID: {}, Label(dataset): {}, Label(kd): {}".format(idx, target, kd_rec['targets']))
        #else:
        #    print('pass')

        return t + (kd_rec['outputs'],)
...
def main():
    ...
    if args.data == "cifar10":
        ...
        train_set = CIFAR10_KD('../data', train=True, download=True,
        ...
        val_set = CIFAR10_KD('../data', train=False,
                                   transform=transforms.Compose([
                                       transforms.ToTensor(),
                                       normalize,
                                   ]))
...
def NLL_loss(pred, y):
    return - torch.sum(pred * torch.log(y) + (1-pred) * torch.log(1-y)) / y.size(0) / y.size(1)

def kd_criterion(outputs, teacher_outputs):
    """
    Source:
        https://github.com/peterliht/knowledge-distillation-pytorch/blob/4e91ea0b3a76d54d1a5d7d29cf51284bc2dbcf4d/model/net.py#L100
    Compute the knowledge-distillation (KD) loss given outputs, labels.
    "Hyperparameters": temperature and alpha
    """
    T = args.T
    loss_fn = nn.KLDivLoss().cuda() #NLL_loss # nn.MultiLabelSoftMarginLoss() #  = nn.KLDivLoss().cuda()
    KD_loss = loss_fn(F.log_softmax(outputs/T, dim=1),
                             F.softmax(teacher_outputs/T, dim=1)) * (T * T)
    return KD_loss

def kd_softmax(inputs, T=10):
    exp_with_T = torch.exp(inputs / T)
    denominator = torch.sum(exp_with_T, dim=1)
    return exp_with_T / denominator.view(-1, 1).expand(-1, 10)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target, kd_target) in enumerate(train_loader):
        ...
        kd_target_var = torch.autograd.Variable(kd_target).cuda()
        ...
        if args.kd:
            hard_loss = (1-args.kd_w) * loss
            #kd_output = kd_softmax(output, T=10)
            kd_loss = args.kd_w * kd_criterion(output, kd_target_var)
            loss = kd_loss + hard_loss
            writer.add_scalar('train/hard_loss', hard_loss.data[0], global_steps)
            writer.add_scalar('train/kd_loss', kd_loss.data[0], global_steps)
        ...
        if args.kd:
            kd_losses.update(kd_loss.data[0], input.size(0))
        top1.update(prec1[0], input.size(0))
        top5.update(prec5[0], input.size(0))

        writer.add_scalar('train/loss', loss.data[0], global_steps)
        writer.add_scalar('train/top1', prec1[0], global_steps)
        writer.add_scalar('train/top5', prec5[0], global_steps)
        ...
        writer.add_scalar('train/batch_time', batch_time.val, global_steps)

        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  'Time {batch_time.val:.3f}\t'  # ({batch_time.avg:.3f}) '
                  'Loss {loss.val:.4f}\t'  # ({loss.avg:.4f}) '
                  'KD_Loss {kd_loss.val: 4f}\t'
                  'Prec@1 {top1.val:.3f}\t'  # ({top1.avg:.3f}) '
                  'Prec@5 {top5.val:.3f}\t'  # ({top5.avg:.3f})'
                  'lr {lr: .4f}'.format(
                      epoch, i, len(train_loader), batch_time=batch_time,
                      loss=losses, kd_loss=kd_losses, top1=top1, top5=top5, lr=lr))
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, target, kd_target) in enumerate(val_loader):
        target = target.cuda(async=True)
        input_var = torch.autograd.Variable(input, volatile=True)
        target_var = torch.autograd.Variable(target, volatile=True)
        kd_target_var = torch.autograd.Variable(kd_target, volatile=True).cuda()
        ...
        if args.kd:
            ...
            kd_losses.update(kd_loss.data[0], input.size(0))
            hard_losses.update(kd_loss.data[0], input.size(0))
        ...
        writer.add_scalar('test/batch_time', batch_time.val, global_steps)
        ...
        if i % args.print_freq == 0:
            print('Test: [{0}/{1}]\t'
                  ...
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                      ...
                      top1=top1, top5=top5, kd_loss=kd_losses))

    print(' * Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f}'
          .format(top1=top1, top5=top5))

    writer.add_scalar('test/loss', losses.avg, global_steps)
    writer.add_scalar('test/top1', top1.avg, global_steps)
    writer.add_scalar('test/top5', top5.avg, global_steps)
    
    if args.kd:
        writer.add_scalar('test/hard_loss', hard_losses.avg, global_steps)
        writer.add_scalar('test/kd_loss', kd_losses.avg, global_steps)
